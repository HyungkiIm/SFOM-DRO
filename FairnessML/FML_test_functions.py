...
from FML_SMD_Solver import *
from FML_FMD_Solver import *
...
def n_num_test(n_num,K_nt, X_train, y_train, c, print_opt,feas_opt,warm_start):
    ...
    # #Check if batchsize is greater than the per-iteration size
    # if K_grad_nt > K_nt:
    #     raise ValueError('Batchsize of SMD should not be greater than ')
...
def K_test_iter(X_train, y_train, c, dual_gap_freq_sst, n_num, K_sst, T_cap_sst, print_opt):
    opt_low_sst = 0  # lower bound of optimal value
    opt_up_sst = 1  # upper bound of optimal value
    obj_tol_sst = 2e-2  # tolerance of our objective value\\ tolerance of R_x and R_i, obj_tol and alg_tol should be same.
    ...
    n, d = X_train.shape
    x_0_sst = np.zeros(d)
    p_0_sst = np.ones((m,n_num)) / n_num
    RHS_sst = np.zeros(3)
    RHS_sst[1:] = c

    print('Problem n = %s' %n_num)
    SMD_stat = DRO_SMD_K_test_iter(x_0_sst, p_0_sst, X_train, y_train, K_sst, K_grad_sst,
