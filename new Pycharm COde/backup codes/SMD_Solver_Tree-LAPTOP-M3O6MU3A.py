...
def SMD_x_tree(K, K_grad, x, step_alpha, K_samples, K_grad_samples, RHS, emp_dist_value, w_sum, cum_dist):
    """
    ...
    for k_idx in range(K_grad):
        g_t += w_sum[i_hat] * emp_dist_value[i_hat, :, :, K_grad_samples[k_idx]] / K_grad
...
def SMD_p(x, p,p_arr, step_alpha, delta, rho, alpha_tol, emp_dist_value, w_sum, w_square_sum):
...
def DRO_SMD_tree(x_0, p_0, emp_dist_value, K, K_grad, delta, rho, alpha_tol, opt_low, opt_up, obj_tol,
    ...
    if K_test_flag == 0:
        ...
        while opt_up - opt_low > obj_tol:
            ...
            for i in range(m):
                bst = RedBlackTree()
                p_tree_list.append(bst.construct_tree(n))
            ...
            for t in range(T):
                ...
                grad_samples_list = []
                for i in range(m):
                    samples_list.append(p_tree_list[i].random_sample(K))
                    grad_samples_list.append(p_tree_list[i].random_sample(K_grad))
                ...
                for i in range(m):
                    w_sum[i], w_square_sum[i] = \
                        SMD_p(x_old, p_tree_list[i], p_arr[i,:], ss_p_list[i][t], delta,rho, alpha_tol, \
                          emp_dist_value_copy[i,:,:,:], w_sum[i], w_square_sum[i])
                ...
                if dual_gap_cal and (t + 2) % dual_gap_freq == 0:
                    x_ws = bar_calculator_temp(x_ws, x, dual_gap_freq, ss_x_list[iter_count * dual_gap_freq:(iter_count+1) * dual_gap_freq])
                    p_ws = bar_calculator_temp(p_ws, t + 2, ss_p_list[0])
                    sup_val = sup_pi(x_bar, emp_dist_value_copy, rho, delta, alpha_tol, RHS)
                    inf_val = inf_pi(p_bar, emp_dist_value_copy, RHS, PWL_model, var_t, var_x, constr_list)
                    diff = sup_val - inf_val
                    dual_gap.append(diff)
