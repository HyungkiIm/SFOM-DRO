...
Here we compare the solution from p_arr and p_list. 
We fix x to using x_arr
...
def SMD_x(K, K_grad, x, step_alpha, samples_list, RHS, emp_dist_value, w_sum, cum_dist):
    """
...
def SMD_p(x, p, i, step_alpha, delta, rho, random_num, emp_dist_value, w_sum, w_square_sum, cum_dist):
    ...
    return p, w_sum, w_square_sum, cum_dist, I_t
...
def DRO_SMD(x_0, p_0, emp_dist_value, K, K_grad, delta, rho, alpha_tol, opt_low, opt_up, obj_tol,
    ...
    if K_test_flag == 0:
        ...
        while opt_up - opt_low > obj_tol:
            ...
            w_sum_arr = np.zeros(m)
            w_sum_arr[:] = np.nan
            w_square_sum_arr = np.zeros(m)
            w_square_sum_arr[:] = np.nan
            cum_dist_arr = np.zeros([m, n])
            cum_dist_arr[:] = np.nan

            for i in range(m):
                w_sum_arr[i] = np.sum(p_0[i, :])
                w_square_sum_arr[i] = np.sum(p_0[i, :] ** 2)
                cum_dist_arr[i, :] = np.cumsum(p_0[i, :])

            w_sum_list = np.zeros(m)
            w_sum_list[:] = np.nan
            w_square_sum_list = np.zeros(m)
            w_square_sum_list[:] = np.nan
            cum_dist_list = np.zeros([m, n])
            cum_dist_list[:] = np.nan

            for i in range(m):
                w_sum_list[i] = np.sum(p_0[i, :])
                w_square_sum_list[i] = np.sum(p_0[i, :] ** 2)
                cum_dist_list[i, :] = np.cumsum(p_0[i, :])
            ...
            for t in range(T):
                ...
                x_arr[t+1,:,:] = SMD_x(K, K_grad, x_arr[t,:,:], ss_x_list[t], temp_samples_list[:, :, t % sample_freq], \
                              RHS, emp_dist_value_copy, w_sum_list, cum_dist_list)

                if dual_gap_freq != 0:

                    for i in range(m):
                        random_num = np.random.rand(1)
                        if (p_arr[t,i,:] != p_list[t][i,:]).any():
                            raise ValueError("Input Values are different.")

                        p_arr[t+1,i,:], w_sum_arr[i], w_square_sum_arr[i], cum_dist_arr[i, :],I_t_arr = SMD_p(x_arr[t,:,:], p_arr[t,i,:], i,
                                                                                     ss_p_list[i][t],
                                                                                     delta,
                                                                                     rho, random_num, \
                                                                                     emp_dist_value_copy, w_sum_arr[i],
                                                                                     w_square_sum_arr[i], cum_dist_arr[i, :])
                        p_tmp[i, :], w_sum_list[i], w_square_sum_list[i], cum_dist_list[i, :], I_t_list = SMD_p(x_arr[t, :, :], p_list[t][i, :],
                                                                                       i,
                                                                                       ss_p_list[i][t],
                                                                                       delta,
                                                                                       rho, random_num, \
                                                                                       emp_dist_value_copy, w_sum_list[i],
                                                                                       w_square_sum_list[i], cum_dist_list[i, :])
                        # print('I_t_arr: %s' %I_t_arr)
                        # print('I_t_list: %s' % I_t_list)
                    ...
                    p_list.append(p_tmp.copy())
                    first_idx, second_idx = np.where(p_list[t + 1] != p_arr[t + 1, :, :])
                    for k in range(first_idx.shape[0]):
                        print('!!!!!!!!!!!!!')
                        print('element of p_list on (%s, %s): %s' %(first_idx[k],second_idx[k],p_list[t+1][first_idx[k],second_idx[k]]))
                        print('element of p_arr on (%s, %s): %s' % (
                        first_idx[k], second_idx[k], p_arr[t + 1,first_idx[k], second_idx[k]]))
                        print("!!!!!!!!!!!!!")

                    #print(np.sum(p_list[t+1] != p_arr[t+1,:,:]))

                    #print(p_arr[1,4,999])

                    if (p_list[t+1] != p_arr[t+1, :, :]).any():
                        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        print(t + 1)
                        print('iteration: %s' % t)
                        print('p_list:', p_list[t + 1])
                        print('p_arr:', p_arr[t + 1, :, :])
                        print('!!!!!!!!!!!!!!!!!!!!!!!')
                ...
                if dual_gap_freq != 0 and t % dual_gap_freq == 0:
                    ...
                    #arr
                    if t!=0:
                        # print(p_list[0][4,999])
                        # print(p_list[1][4, 999])
                        # print(p_list[5][4,999])
                        # print(p_list[1001][4, 999])
                        # p_bar_list = bar_calculator_list(p_list[1:], t + 1, ss_p_list[0])
                        # # We got to know that except p[0], all the values are set to same.
                        # print(p_bar_list[4,999])
                        #p_list_temp = np.asarray(p_list)

                        for i in range(t+2):
                            if (p_list[i] != p_arr[i,:,:]).any():
                                print('!=====================================')
                                first_idx, second_idx = np.where(p_list[i] != p_arr[i, :, :])
                                for k in range(first_idx.shape[0]):
                                    if abs(p_list[i][first_idx[k], second_idx[k]] - p_arr[i, first_idx[k], second_idx[k]]) > 1e-7:

                                        print('!!!!!!!!!!!!!')


                                        print('element of p_list on t= %s (%s, %s): %s' % (i,
                                        first_idx[k], second_idx[k], p_list[i][first_idx[k], second_idx[k]]))
                                        print('element of p_arr on (%s, %s): %s' % (
                                            first_idx[k], second_idx[k], p_arr[i, first_idx[k], second_idx[k]]))
                                        print("!!!!!!!!!!!!!")

                                raise ValueError('ASDASDASDASDASDASDASDASD')
                    ...
                    # list option output
                    x_bar_arr = bar_calculator_arr(x_arr, t + 2, ss_x_list)
                    p_bar_list = bar_calculator_list(p_list, t + 2, ss_p_list[0])


                    if (p_bar_arr!=p_bar_list).any():
                        raise ValueError('Bar calculation are different')

                    sup_val_arr = sup_pi(x_bar_arr, emp_dist_value_copy, rho, delta, alpha_tol, RHS)
                    inf_val_list = inf_pi(p_bar_list, emp_dist_value_copy, RHS, PWL_model, var_t, var_x, constr_list)
                    diff_list = sup_val_arr - inf_val_list

                    if diff_arr != diff_list:
                        print('inf_val_arr: %s' %inf_val_arr)
                        print('inf_val_list: %s' %inf_val_list)
                        raise ValueError("Different Difference")
                    else:
                        dual_gap.append(diff_list)
                ...
                if dual_gap_freq != 0 and t % dual_gap_freq == 0 and diff_list <= obj_tol / 2:
                    break_flag = 1
                    iter_toc = time.time()
                ...
                if dual_gap_freq != 0 and t == T - 1 and print_option:
                    x_bar = bar_calculator(x_arr, t + 2, ss_x_list)
                    p_bar = bar_calculator(p_arr, t + 2, ss_p_list[0])
                    sup_val = sup_pi(x_bar, emp_dist_value_copy, rho, delta, alpha_tol, RHS)
                    inf_val = inf_pi(p_bar, emp_dist_value_copy, RHS, PWL_model, var_t, var_x, constr_list)
                    diff = sup_val - inf_val
                    dual_gap.append(diff)
