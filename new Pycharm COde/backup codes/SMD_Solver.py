...
4. Then what about constant case? 
5. Frequent update of x and p and x_bar and p_bar --> Complete this today. 
"""

"""
Here we use following structure to save our previous solutions. 
x = np.empty([T+1,J,L])
p = np.empty([T+1,m,n])
    ...
    if K_test_flag == 0:
        ...
        while opt_up - opt_low > obj_tol:
            ...
            for t in range(T):
                ...
                x[t+1,:,:] = SMD_x(K, K_grad, x[t,:,:], ss_x_list[t], temp_samples_list[:, :, t % sample_freq], \
                              grad_samples_list[:,t % sample_freq],RHS, emp_dist_value_copy, w_sum, cum_dist)

                if dual_gap_freq != 0:

                    for i in range(m):
                        p[t+1,i,:], w_sum[i], w_square_sum[i], cum_dist[i, :] = SMD_p(x[t,:,:], p[t,i,:], i,
                                                                                     ss_p_list[i][t],
                                                                                     delta,
                                                                                     rho, alpha_tol, \
                    ...
                    for i in range(m):
                        ...
                        p_old[i,:], w_sum[i], w_square_sum[i], cum_dist[i, :] = SMD_p(x[t,:,:], p_old[i, :], i,
                                                                                       ss_p_list[i][t],
                                                                                       delta,
                                                                                       rho, alpha_tol, \
                                                                                       emp_dist_value_copy, w_sum[i],
