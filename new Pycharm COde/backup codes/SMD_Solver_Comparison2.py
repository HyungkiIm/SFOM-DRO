...
We use p_list here.
After comparison with SMD_Solver_Comparison, we get to know that using p_arr was the problem. 
        ...
        while opt_up - opt_low > obj_tol:
            ...
            for t in range(T):
                ...
                if dual_gap_freq != 0:
                    ...
                    for i in range(m):
                        p_tmp[i,:], w_sum[i], w_square_sum[i], cum_dist[i, :] = SMD_p(x_arr[t,:,:], p_list[t][i,:], i,
                                                                                     ss_p_list[i][t],
                                                                                     delta,
                                                                                     rho, alpha_tol, \
                ...
                if dual_gap_freq != 0 and t % dual_gap_freq == 0:
                    ...
                    diff_arr = sup_val_arr - inf_val_list
                    ...
                    inf_val_list = inf_pi(p_bar_list, emp_dist_value_copy, RHS, PWL_model, var_t, var_x, constr_list)
                    diff_list = sup_val_list - inf_val_list
                ...
                if dual_gap_freq != 0 and t % dual_gap_freq == 0 and diff_list <= obj_tol / 2:
                    ...
                    if print_option:
                        ...
                        print('x_bar:', x_bar_list)
                        # print('p_bar:', p_bar)
                        print('Duality Gap:', diff_list)
                        print("=============================================")
                    if pi_val(x_bar_list, p_bar_list, emp_dist_value_copy, RHS) > obj_tol / 2:
                        if min_flag:
                            opt_low = obj_val
