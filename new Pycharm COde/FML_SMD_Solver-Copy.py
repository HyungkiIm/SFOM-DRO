...
K: per-iteration Sample size of our gradient 
...
def SMD_x(K, K_grad, x, step_alpha, samples_list, grad_samples_list, X_train, y_train, s_norm, RHS, w_sum, cum_dist):
    """
...
def SMD_x_K_test(K, K_grad,C_K,obj_tol, x, p, step_alpha, samples_list, grad_samples_list, X_train, y_train, s_norm,
    ...
    if i_hat == 0:
        for k_idx in range(K_grad):
            temp = np.exp(X_train[K_grad_samples[k_idx],:] @ x)
            g_t += w_sum[i_hat] * X_train[K_grad_samples[k_idx],:] * (temp/(1+temp) - 1 + y_train[K_grad_samples[k_idx]]) / K_grad
    elif i_hat == 1:
        for k_idx in range(K_grad):
            g_t += w_sum[i_hat] * s_norm[K_grad_samples[k_idx]] * X_train[K_grad_samples[k_idx],:] / K_grad
    else:
        for k_idx in range(K_grad):
            g_t += -w_sum[i_hat] * s_norm[K_grad_samples[k_idx]] * X_train[K_grad_samples[k_idx],:] / K_grad
    ...
    if K_test_flag == 0:
        ...
        while opt_up - opt_low > obj_tol and not feas_flag:
            ...
            for t in range(T):
                ...
                #hat_vartheta sanity check (update later)

                # if vartheta_flag:
                #     real_F_hat = []
                #     for i in range(m):
                #         temp = 0
                #         temp = np.sum(np.multiply(np.tensordot(p[t%dual_gap_freq,i, :], emp_dist_value_copy,\
                ...
                x[(t+1)%dual_gap_freq,:],hat_f_val[t%dual_gap_freq,:] = SMD_x(K, K_grad, x[t%dual_gap_freq,:],\
                    ss_x_list[t], temp_samples_list[:, :, t % sample_freq], grad_samples_list[:,t % sample_freq],
                    X_train, y_train, s_norm, RHS, w_sum, cum_dist)
                for i in range(m):
...
def DRO_SMD_K_test_iter(x_0, p_0, X_train, y_train, K, K_grad, delta, \
    ...
    dual_gap = []  # List that contains duality gap in this bisection
    #We do not calculate
    # sup_val = sup_pi(c_vec, s_vec, r_vec, b_vec, beta,x_0, emp_dist_value_copy, rho, delta, alpha_tol, RHS)
    # inf_val = inf_pi(c_vec, s_vec, r_vec, b_vec, beta, p_0, emp_dist_value, RHS, opt_model, var_x, var_y, var_tau,
