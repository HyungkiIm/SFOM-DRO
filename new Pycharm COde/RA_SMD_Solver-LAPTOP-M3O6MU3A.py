...
from RA_UBRegret import R_dim_SMD
from RA_utils import *
...
def SMD_x(K, K_grad, x, step_alpha, samples_list, grad_samples_list, RHS, emp_dist_value, w_sum, cum_dist):
    ...
    for i in range(m):
        temp = 0
        temp = w_sum[i] * np.sum(np.tensordot(x,emp_dist_value[i,:,:][:,K_samples[i, :].astype(int)], axes = (0,0)) ** 2)
    ...
    if i_hat == 0:
        for k_idx in range(K_grad):
            g_t += 2 * w_sum[i_hat] * emp_dist_value[i_hat,:,int(K_grad_samples[k_idx])] *\
                   (np.dot(x,emp_dist_value[i_hat,:,int(K_grad_samples[k_idx])])) / K_grad
    elif i_hat == 1:
        for k_idx in range(K_grad):
...
def SMD_x_K_test(K, K_grad,C_K,obj_tol, x, p, step_alpha, samples_list, grad_samples_list, RHS, emp_dist_value, w_sum, cum_dist):
    ...
    for i in range(m):
        temp = 0
        temp = np.dot(p[i,:],np.tensordot(x,emp_dist_value[i,:,:], axes = (0,0)) ** 2)
...
def DRO_SMD(x_0, p_0, emp_dist_value, K, K_grad, delta, rho, alpha_tol, opt_low, opt_up, obj_tol,
    ...
    """
    emp_dist_value_copy1 = deepcopy(emp_dist_value)
    emp_dist_value_copy2 = deepcopy(-emp_dist_value)
    emp_dist_value_copy = np.stack((emp_dist_value_copy1,emp_dist_value_copy2))
    emp_dist_value_copy[0, :, :] = (-1 + 2 * min_flag) * emp_dist_value_copy[0, :, :]
    m, d, n = emp_dist_value_copy.shape  # Parameters
    ...
    G = 2 * np.absolute(emp_dist_value).max()
    M = []
    M.append(4 * np.absolute(emp_dist_value).max() ** 2)
    M.append(np.absolute(emp_dist_value).max())

    #Start from changing this T calculation function 02-04

    # Calculate T and our stepsize
    # if ss_type == 'constant':
    #     T, R_x, R_p, ss_x, ss_p = R_const_SMD(d, n, G, M, delta, rho, obj_tol, C_G, C_K, stoc_factor)
    #     if K_test_flag:
    ...
    var_x = PWL_model.addVars(list_d, ub=1, name='x')
    PWL_model.addConstr(gp.quicksum(var_x[l] for l in list_d) == 1)
