...
class Statistics:
    def __init__(self, n, m, K, K_grad, ss_type, last_obj_val, dual_gap_list, \
                 ...
                 bound_T, R_x, R_p, i_flag_count,dual_gap_time = 0):
        self.n = n
        self.m = m
        self.K = K
        self.K = K
...
def bar_calculator(prev_x_ws, x, total_iter, ss_list,ss_sum):
...
def dual_gap_calculator(x_ws,x, dual_gap_freq,ss_x_list,ss_sum_x,
    ...
    x_ws, ss_sum_x = bar_calculator(x_ws,x,dual_gap_freq,ss_x_list,ss_sum_x)
    last_p = p[-1,:,:].copy()
    ...
    actual_p = p
    p_ws, ss_sum_p = bar_calculator(p_ws,actual_p,dual_gap_freq,ss_p_list,ss_sum_p)
