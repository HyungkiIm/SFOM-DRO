...
def R_dim_SMD(d, n, G, M, delta, rho, obj_tol, C_g, C_K, C_s):
    ...
    M_max = max(M)
    G_x = C_g * G
    D_x = np.log(d)  # Finite diameter of our primal space
    M = np.asarray(M)
    G_p = M * C_g * n / delta
...
def R_dim_FMD(d, n, G, M, rho, obj_tol):
    M_max = max(M)
    D = np.log(d)  # Finite diameter of our primal space
    C2 = 2 + math.sqrt(3)
    iter_ub = 1000
    iter_lb = 1
    ...
    c_p = []
    c_x = math.sqrt(2 * np.log(d)) / (G * math.sqrt(np.sum(1 / (np.arange(T) + 1))))
